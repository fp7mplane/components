...
#sys.path.append(os.getcwd()+'/fastping/')

from fastping.Fastping import Fastping
...
parser.add_argument('-d','--deltaM',type=int,default=10,help='cycle duration')#default 300
...
#FolderSetting
parser.add_argument('-p','--filePath',type=str,default='./',help='path for output file')
...
parser.add_argument('-U','--upload', nargs=6,help='Upload result on server')
...
if values.targetList!=None:
    try:
        targetFile = open("iplist.dat",'w')
        target="iplist.dat" #it will be iplist.dat
    ...
    try:
        target="iplist3.dat" #it will be iplist.dat
